...
router.put(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (req.body.status === "Chuyển giao cho đối tác giao hàng") {
...
router.put(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (!order) {
        return next(new ErrorHandler("Không tìm thấy đơn hàng với mã đơn hàng này!", 400));
      }
