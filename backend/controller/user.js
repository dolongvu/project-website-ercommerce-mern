...
router.post("/create-user", upload.single("file"), async (req, res, next) => {
  try {
    ...
    if (userEmail) {
      ...
      return next(new ErrorHandler("Người dùng này đã được đằng ký", 400));
    }
...
router.delete(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (!user) {
        return next(
          new ErrorHandler("Không có người dùng với id này", 400)
        );
      }
      ...
      res.status(201).json({
        success: true,
        message: "Xóa người dùng này thành công!",
      });
    } catch (error) {
      return next(new ErrorHandler(error.message, 500));
    }
